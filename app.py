...
# --- INISIALISASI STATE (Gembok agar tidak error) ---
...
# ==================================
# Bagian 1: CSS CUSTOM (WARNA SESUAI INSTRUKSI)
# ==================================
st.markdown("""
<style>
    ...
    .tile-green-light { background: linear-gradient(135deg, #98ec2d 0%, #17ad4a 100%); } /* HIJAU MUDA */
...
def load_all_data():
    try:
        ...
        for df in [df_ats, df_sch]: df.columns = df.columns.str.strip()
...
st.sidebar.divider()
opsi_kab = ["Semua"] + sorted(data_wilayah["kab_kota"].unique().tolist()) if not data_wilayah.empty else ["Semua"]
...
if st.session_state.page_view == "dashboard":
    ...
    val_p = f"{int(df_f['jumlah_penduduk'].sum()):,}" if not df_f.empty else "0"
    val_s = f"{int(df_f['jumlah_siswa'].sum()):,}" if not df_f.empty else "0"
    val_a = f"{int(df_f['ats_disabilitas'].sum()):,}" if not df_f.empty else "0"
    val_APS = f"{(df_f['jumlah_siswa'].sum() / df_f['jumlah_penduduk'].sum() * 100):.2f}%" if not df_f.empty and df_f['jumlah_penduduk'].sum() > 0 else "0%"
    ...
    if kab_pilih != "Semua":
        ...
        sch_wil = data_sekolah[data_sekolah['kab_kota'] == kab_pilih] if not data_sekolah.empty else pd.DataFrame()
    ...
    with cv2:
        ...
        if not df_f.empty: st.plotly_chart(px.bar(df_f.sort_values("ats_disabilitas", ascending=False).head(10), x='ats_disabilitas', y='kab_kota', orientation='h', color_continuous_scale='Blues'), use_container_width=True)
