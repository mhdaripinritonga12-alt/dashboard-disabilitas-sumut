...
# ==================================
# Bagian 1: CSS CUSTOM (ULTRA SLIM & MODERN)
# ==================================
st.markdown("""
<style>
    ...
    section[data-testid="stSidebar"] .stButton button {
        background-color: rgba(255, 255, 255, 0.1) !important; color: white !important; border: 1px solid rgba(255, 255, 255, 0.2) !important;
        border-radius: 8px !important; width: 100% !important; text-align: left !important; padding: 2px 15px !important; font-size: 13px !important; min-height: 32px !important; margin-bottom: 2px !important;
    }
    ...
    .tile-green-light { background: linear-gradient(135deg, #4caf50, #2e7d32); }

    /* KOTAK INSIGHT (STYLE BARU) */
    .insight-box {
        background-color: #f8f9fa !important; border-radius: 10px; border-left: 6px solid #0d47a1; padding: 15px; margin-top: 15px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);
    }
    .insight-title { color: #0d47a1; font-weight: 800; font-size: 14px; text-transform: uppercase; margin-bottom: 8px; display: flex; align-items: center; gap: 8px; }
    .insight-text { color: #333 !important; font-size: 13px; line-height: 1.6; margin: 0; }

    /* SOURCE BOX ORENS */
    .source-box-ui { background-color: #fff3e0 !important; padding: 5px 12px; border-radius: 8px; border-left: 5px solid #ff9800; margin-bottom: 15px; width: 100%; display: block !important; box-sizing: border-box; }
</style>
...
with st.sidebar:
    ...
    if logo_b64:
        st.markdown(f'<div style="display:flex;align-items:center;gap:10px;padding-bottom:10px;"><img src="data:image/png;base64,{logo_b64}" width="35"><span style="font-size:16px;font-weight:800;color:white;">SI-PANDAI SUMUT</span></div>', unsafe_allow_html=True)
    ...
    if st.button("🚀 Dashboard Utama", key="nav_dash", use_container_width=True): st.session_state.page_view = "dashboard"; st.rerun()
    if st.button("🎓 Pendidikan Khusus", key="nav_pk", use_container_width=True): st.session_state.page_view = "tentang_pk"; st.rerun()
    if st.button("ℹ️ Tentang Dashboard", key="nav_about", use_container_width=True): st.session_state.page_view = "tentang_dashboard"; st.rerun()
...
st.markdown("""
    <div class="header-balloon-card">
        ...
        <p style='color: #1565c0; font-size: 13px; font-weight: 700; margin: 0;'>Sistem Informasi Pemetaan Anak Tidak Sekolah Disabilitas Sumatera Utara</p>
    </div>
""", unsafe_allow_html=True)
...
if st.session_state.page_view == "dashboard":
    ...
    st.markdown("""<div class="source-box-ui"><p style="font-size: 11px; color: #e65100; margin: 0; font-weight: 700;"><b>ℹ️ Sumber Data:</b> Bidang PK, Data LPPD Disdik & TIKP Provsu 2025</p></div>""", unsafe_allow_html=True)
    ...
    if not df_f.empty:
        v_p = f"{int(df_f.iloc[:,1].sum()):,}"; v_s = f"{int(df_f.iloc[:,2].sum()):,}"; v_a = f"{int(df_f.iloc[:,3].sum()):,}"
        v_aps = f"{(int(df_f.iloc[:,2].sum()) / int(df_f.iloc[:,1].sum()) * 100):.2f}%" if int(df_f.iloc[:,1].sum()) > 0 else "0%"
    else: v_p = v_s = v_a = "0"; v_aps = "0%"

    with m1: draw_tile_svg("Penduduk Disabilitas", v_p, svg_people, "tile-orange")
    with m2: draw_tile_svg("Siswa Belajar", v_s, svg_cap, "tile-blue-light")
    with m3: draw_tile_svg("Anak Tidak Sekolah", v_a, svg_warning, "tile-red-dark")
    ...
    with cv2:
        ...
        if not df_f.empty:
            ...
            fig = px.bar(
                ...
                orientation='h',
                color=col_kab,
                text=ats_col, # Memunculkan Angka
                color_discrete_sequence=px.colors.qualitative.Pastel
            )
            fig.update_traces(textposition='outside', textfont_size=12, textfont_color="black", texttemplate='%{text:,}')
            fig.update_layout(showlegend=False, height=350, margin=dict(t=10, b=10, l=10, r=40), xaxis_title="Jumlah ATS", yaxis_title="")
            st.plotly_chart(fig, use_container_width=True)
            
            # KOTAK INSIGHT DYNAMIS
            top_names = ", ".join(df_top5[col_kab].tolist())
            st.markdown(f"""
                <div class="insight-box">
                    <div class="insight-title">💡 Insight Dashboard</div>
                    <p class="insight-text">
                        Berdasarkan data terbaru, 5 wilayah dengan jumlah Anak Tidak Sekolah (ATS) tertinggi adalah <b>{top_names}</b>. 
                        Total ATS di wilayah pilihan saat ini mencapai <b>{v_a}</b> jiwa dengan rata-rata partisipasi <b>{v_aps}</b>. 
                        Diperlukan prioritas intervensi pada daerah-daerah peringkat atas tersebut.
                    </p>
                </div>
            """, unsafe_allow_html=True)
    ...
    st.write("Sistem Informasi SI-PANDAI.")
