...
<style>
    ...
    .tile-green-light { background: linear-gradient(135deg, #98ec2d 0%, #17ad4a 100%); } /* HIJAU MUDA GRADASI */
...
# =========================
# Bagian 3: LOGIN (ORIGINAL 2 COLS)
# =========================
...
st.sidebar.divider()
kab_opsi = ["Semua"] + sorted(data_wilayah["kab_kota"].unique().tolist()) if not data_wilayah.empty else ["Semua"]
kab_pilih = st.sidebar.selectbox("Pilih Kabupaten / Kota", kab_opsi, key="selected_kab")
st.sidebar.divider()
...
if st.session_state.page_view == "dashboard":
    ...
    with cv2:
        ...
        if not df_f.empty: st.plotly_chart(px.bar(df_f.sort_values("ats_disabilitas", ascending=False).head(10), x='ats_disabilitas', y='kab_kota', orientation='h', color_discrete_sequence=['#0d47a1']), use_container_width=True)
