...
if st.session_state.page_view == "dashboard":
   ...
   # Matriks (Hanya 3 Kotak sesuai Database GitHub)
    ...
    if not df_f.empty:
        # 1. PERBAIKAN DATA (Diletakkan di luar fungsi peta)
        # Menghapus titik ribuan agar terbaca sebagai angka bulat
        df_f['ats'] = pd.to_numeric(df_f['ats'].astype(str).str.replace('.', ''), errors='coerce').fillna(0).astype(int)
        
        # 2. DEFINISI NAMA KOLOM (Sesuaikan dengan file GitHub Anda)
        # Di GitHub Anda kolomnya bernama 'lat', 'lon', dan 'kab_kota'
        col_lat = "lat"
        col_lon = "lon"
