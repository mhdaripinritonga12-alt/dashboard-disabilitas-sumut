...
# ==================================
# Bagian 1: CSS CUSTOM (FIX KONSOLIDASI)
# ==================================
st.markdown("""
<style>
    ...
    .insight-title { color: #0d47a1; font-weight: 800; font-size: 14px; text-transform: uppercase; margin-bottom: 5px; display: flex; align-items: center; gap: 8px; }
    .insight-text { color: #333 !important; font-size: 13px; line-height: 1.5; margin: 0; }
    ...
    /* SIDEBAR BUTTONS ULTRA SLIM */
    section[data-testid="stSidebar"] .stButton button { background-color: rgba(255, 255, 255, 0.1) !important; color: white !important; border-radius: 8px !important; width: 100% !important; text-align: left !important; padding: 2px 15px !important; font-size: 13px !important; }
    
    /* EXPANDER & DOWNLOAD GREEN */
...
# ==================================
# Bagian 2: LOAD DATA (SINGLE CALL)
# ==================================
@st.cache_data
def load_all_data():
    try:
...
with st.sidebar:
    ...
    if st.button("🎓 Pendidikan Khusus", key="nav_pk", use_container_width=True):
        st.session_state.page_view = "tentang_pk"; st.rerun()
    if st.button("ℹ️ Tentang Dashboard", key="nav_about", use_container_width=True):
        st.session_state.page_view = "tentang_dashboard"; st.rerun()
...
st.markdown("""
    <div class="header-balloon-card">
        ...
        <div style="height: 2px; background: linear-gradient(90deg, transparent, #0d47a1, transparent); margin: 3px auto; width: 50%; opacity: 0.3;"></div>
        <p style='color: #1565c0; font-size: 14px; font-weight: 700; margin: 0;'>Sistem Informasi Pemetaan Anak Tidak Sekolah Disabilitas Sumatera Utara</p>
    </div>
""", unsafe_allow_html=True)
...
if st.session_state.page_view == "dashboard":
    st.markdown('<p style="font-size:26px; font-weight:800; color:#0d47a1; margin-bottom:5px;">Matriks Capaian Sektoral</p>', unsafe_allow_html=True)
    st.markdown("""<div class="source-box-ui"><p style="font-size: 11px; color: #e65100; margin: 0; font-weight: 700;"><b>ℹ️ Sumber Data:</b> Bidang PK, LPPD Disdik & TIKP Provsu 2025</p></div>""", unsafe_allow_html=True)
