...
# Inisialisasi State (Hanya Satu Kali)
...
<style>
    ...
    .gradient-line-inner {
        height: 2px; background: linear-gradient(90deg, transparent, #0d47a1, transparent);
        margin: 2px auto; width: 50%; opacity: 0.3;
    }

    /* SIDEBAR & FIX TEKS HITAM FILTER */
    [data-testid="stSidebar"] { background: linear-gradient(180deg, #1e88e5 0%, #0d47a1 100%) !important; }
    [data-testid="stSidebar"] * { color: white !important; }

    /* Memaksa teks didalam kotak putih menjadi hitam */
    ...
    /* EXPANDER & DOWNLOAD (HIJAU GRADASI) */
    ...
    div.stButton > button[key^="btn_"] {
        ...
        color: white !important; border-radius: 20px !important; font-weight: 700 !important;
    }
</style>
...
with st.sidebar:
    ...
    pilihan = st.radio("Navigasi:", ["🚀 Dashboard Utama", "🎓 Pendidikan Khusus", "ℹ️ Tentang Dashboard"], key="nav_radio")
...
if st.session_state.page_view == "dashboard":
            ...
            for i, row in enumerate(sch_wil.itertuples()):
                with cols[i % 3]:
                    with st.container(border=True):
                        ...
                        if st.button("DETAIL", key=f"btn_{i}", use_container_width=True):
    ...
    with st.expander("📋 Lihat & Download Data Tabel"):
        ...
        st.download_button("Download Data sebagai CSV 📥", csv, file_name=f'data_sipandai_{kab_pilih}.csv', mime='text/csv')

elif st.session_state.page_view == "detail":
    sch = st.session_state.selected_school_data
    st.markdown(f"### 🏫 {sch['nama_sekolah'].upper()}")
    st.write(f"NPSN: {sch['npsn']} | Wilayah: {sch['kab_kota']}")
    st.info("Visualisasi Peta Sekolah")
    st.divider()
