...
# ==================================
# Bagian 1: CSS CUSTOM (FIX HIJAU MUDA & SLIM)
# ==================================
st.markdown("""
<style>
    ...
    .insight-box { background-color: #e8f5e9 !important; border-radius: 10px; border-left: 6px solid #0d47a1; padding: 15px; margin-top: 15px; }
    .insight-title { color: #2e7d32; font-weight: 800; font-size: 14px; text-transform: uppercase; margin-bottom: 5px; }
    ...
    div[data-testid="stExpander"] details summary { background: linear-gradient(90deg, #4caf50 0%, #2e7d32 100%) !important; color: white !important; border-radius: 10px !important; padding: 10px 15px !important; }
...
# ==================================
# Bagian 4: SIDEBAR & NAVIGASI (FIX ERROR)
# ==================================
def ubah_halaman():
    # Proteksi: Hanya jalankan jika widget nav_radio ada di state
    pilihan = st.session_state.get("nav_radio")
    if pilihan:
...
with st.sidebar:
    ...
    st.radio("Navigasi:", ["🚀 Dashboard Utama", "🎓 Pendidikan Khusus", "ℹ️ Tentang Dashboard"], key="nav_radio", on_change=ubah_halaman, label_visibility="collapsed")
...
if st.session_state.page_view == "dashboard":
    ...
    with cv1:
        ...
        if not df_f.empty:
            ...
            fig_map = px.scatter_mapbox(df_f, lat="lat", lon="lon", size=ats_col, color=ats_col, color_continuous_scale="RdYlGn_r", zoom=7, height=450)
            fig_map.update_layout(mapbox_style="open-street-map", margin={"r":0,"t":0,"l":0,"b":0}, coloraxis_showscale=False)
            st.plotly_chart(fig_map, use_container_width=True)
            st.markdown("""<div style="display: flex; gap: 15px; margin-top: -50px; margin-left: 10px; position: relative; z-index: 999; background: rgba(255,255,255,0.9); padding: 8px 12px; border-radius: 8px; width: fit-content; border: 1px solid #ddd;"><div style="display: flex; align-items: center; gap: 6px;"><div style="width: 12px; height: 12px; background-color: #1a9641; border-radius: 50%;"></div><span style="font-size: 11px; font-weight: 800;">RENDAH</span></div><div style="display: flex; align-items: center; gap: 6px;"><div style="width: 12px; height: 12px; background-color: #ffffbf; border-radius: 50%; border: 1px solid #ccc;"></div><span style="font-size: 11px; font-weight: 800;">SEDANG</span></div><div style="display: flex; align-items: center; gap: 6px;"><div style="width: 12px; height: 12px; background-color: #d7191c; border-radius: 50%;"></div><span style="font-size: 11px; font-weight: 800;">TINGGI</span></div></div>""", unsafe_allow_html=True)

    with cv2:
        st.subheader("📊 5 Peringkat ATS Tertinggi")
        if not df_f.empty:
            df_top5 = df_f.sort_values(by=ats_col, ascending=False).head(5)
            fig_bar = px.bar(df_top5, x=ats_col, y=col_kab, orientation='h', color=col_kab, color_discrete_sequence=['#800000', '#008000', '#FF8C00', '#00008B', '#ADD8E6'], text=ats_col)
            fig_bar.update_layout(height=300, bargap=0.4, showlegend=False, plot_bgcolor='rgba(0,0,0,0)', xaxis_title=None, yaxis_title=None)
            st.plotly_chart(fig_bar, use_container_width=True)
            
            jml_sekolah = len(data_sekolah[data_sekolah[col_kab] == kab_pilih]) if kab_pilih != "Semua" else len(data_sekolah)
            # Logika Warna Aksen
            w_box = "#b71c1c" if v_a > 0 and jml_sekolah == 0 else "#2e7d32" if v_a == 0 else "#ef4444" if v_a > 100 else "#0d47a1"
            txt_i = f"🚨 PERINGATAN KRITIS: Di {kab_pilih} ada {v_a:,} ATS, tapi BELUM ADA SLB." if v_a > 0 and jml_sekolah == 0 else f"✅ {kab_pilih} bersih dari ATS." if v_a == 0 else f"⚠️ ATS di {kab_pilih} tinggi ({v_a:,} jiwa)." if v_a > 100 else f"💡 Wilayah {kab_pilih} memiliki {v_a:,} ATS."
            
            st.markdown(f'<div class="insight-box" style="border-left: 6px solid {w_box};"><div class="insight-title" style="color:{w_box}">💡 Insight & Rekomendasi: {kab_pilih}</div><p class="insight-text">{txt_i}</p><hr style="opacity:0.2;"><p class="insight-text" style="font-weight:700; color:{w_box}">Tindakan: Diperlukan langkah strategis sesuai data.</p></div>', unsafe_allow_html=True)
