...
if st.session_state.page_view == "dashboard":
    ...
    # --- TAMPILAN HALAMAN DETAIL SEKOLAH (FIXED) ---
    ...
    st.markdown(f"<h1 style='color: #0d47a1; margin-bottom: 0px;'>🏫 {sch['nama_sekolah'].upper()}</h1>", unsafe_allow_html=True)
    ...
    with col_d1:
        with st.container(border=True):
            ...
            st.write(f"**Status Sekolah:** {sch.get('status', 'Negeri')}")
    ...
    with col_d2:
        with st.container(border=True):
            ...
            st.write(f"**Jumlah Rombel:** {sch.get('jumlah_rombel', 0)}")
            st.write(f"**Jumlah Ruang Kelas:** {sch.get('jumlah_ruang_kelas', 0)}")
