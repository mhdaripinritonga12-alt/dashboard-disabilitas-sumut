...
# ==================================
# Bagian 1: CSS CUSTOM (LOCKED & FIXED UI)
# ==================================
st.markdown("""
<style>
...
if st.session_state.page_view == "dashboard":
    ...
    # 3. Visualisasi (PETA GIS SEPERTI GAMBAR)
    st.divider()
    cv1, cv2 = st.columns([1.6, 1])
    with cv1:
        st.subheader("🗺️ Peta Pemetaan ATS Disabilitas")
        
        # UPGRADE PETA KE PLOTLY SCATTER MAPBOX
        fig_map = px.scatter_mapbox(
            df_filter,
            lat="lat",
            lon="lon",
            size="ats_disabilitas",
            color="ats_disabilitas",
            color_continuous_scale="Reds",
            size_max=25,
            zoom=7,
            hover_name="kab_kota",
            hover_data={"ats_disabilitas": True, "lat": False, "lon": False},
            mapbox_style="carto-positron" # Desain peta bersih & profesional
        )
        fig_map.update_layout(
            margin={"r":0,"t":0,"l":0,"b":0},
            coloraxis_showscale=False # Sembunyikan colorbar agar lebih rapi
        )
    ...
    with cv2:
        ...
        fig_bar = px.bar(df_chart, x='ats_disabilitas', y='kab_kota', orientation='h', 
                         color='ats_disabilitas', color_continuous_scale='Blues')
        fig_bar.update_layout(height=400, margin=dict(l=0, r=0, t=0, b=0), showlegend=False)
