...
st.sidebar.divider()
opsi = ["Semua"] + sorted(data_wilayah["kab_kota"].unique().tolist()) if not data_wilayah.empty else ["Semua"]
...
if st.session_state.page_view == "dashboard":
    ...
    with cv1:
        # --- FITUR: PETA SEBARAN SEKOLAH (MULTI-DOT) ---
        st.subheader(f"🗺️ Peta Lokasi Satuan Pendidikan - {kab_pilih}")
        if kab_pilih == "Semua":
            if not df_f.empty: st.map(df_f, latitude="lat", longitude="lon", use_container_width=True)
        else:
            # Filter sekolah untuk menampilkan semua titik sekaligus dalam satu kabupaten
            df_map_sekolah = data_sekolah[data_sekolah['kab_kota'] == kab_pilih].copy()
            if not df_map_sekolah.empty:
                st.map(df_map_sekolah, latitude="latitude", longitude="longitude", use_container_width=True)
            else:
                st.info("Koordinat sekolah di wilayah ini belum tersedia.")

    with cv2:
        st.subheader("📊 Statistik Wilayah")
        if not df_f.empty: st.plotly_chart(px.bar(df_f.head(10), x=df_f.columns[3], y='kab_kota', orientation='h', color_discrete_sequence=['#0d47a1']), use_container_width=True)

    if kab_pilih != "Semua":
        st.subheader(f"🏫 Daftar Sekolah")
            ...
            for i, row in enumerate(sch_wil.itertuples()):
                with cols[i % 3]:
                    with st.container(border=True):
                        ...
                        if st.button(getattr(row, 'nama_sekolah', 'SEKOLAH').upper(), key=f"btn_{row.npsn}"):
    ...
    # Street View Style Map for Detail
    q = f"{sch['nama_sekolah'].replace(' ', '+')}+{sch['kab_kota'].replace(' ', '+')}"
    components.html(f'<iframe width="100%" height="350" frameborder="0" src="https://www.google.com/maps?q={q}&output=embed" style="border-radius:15px;"></iframe>', height=370)
