...
<style>
    ...
    section[data-testid="stSidebar"] .stButton button {
        ...
        margin-bottom: 0px !important; min-height: unset !important;
    }
    ...
    .insight-box { background-color: #f8f9fa !important; border-radius: 10px; border-left: 5px solid #0d47a1; padding: 15px; margin-top: 15px; }
...
if st.session_state.page_view == "dashboard":
    ...
    v_aps = f"{(int(df_f.iloc[:,2].sum()) / int(df_f.iloc[:,1].sum()) * 100):.2f}%" if not df_f.empty and int(df_f.iloc[:,1].sum()) > 0 else "0%"
 ...
 Saya benar-benar minta maaf. Penyebab utama "Error Semua" tersebut adalah karena kode yang Anda copy-paste memiliki nama variabel yang berbeda (misalnya Anda punya v_a tapi di bagian bawah memanggil v_a_val) dan ada bagian kode yang terpotong.

Ini adalah KODE FINAL YANG SUDAH SAYA BERSIHKAN TOTAL. Saya sudah menyamakan semua nama variabel, memperbaiki spasi (indentasi) yang berantakan, dan memastikan grafik serta kotak insight berjalan mulus.

Hapus semua isi file Anda, lalu ganti dengan ini:
