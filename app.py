...
if st.session_state.page_view == "dashboard":
    ...
    with cv1:
        # --- PETA MULTI-DOT SEKOLAH ---
        st.subheader(f"🗺️ Sebaran Sekolah: {kab_pilih}")
    ...
    # Unit Pendidikan
    if kab_pilih != "Semua":
        st.subheader(f"🏫 Satuan Pendidikan")
            ...
            for i, row in enumerate(sch_wil.itertuples()):
                with cols[i % 3]:
                    with st.container(border=True):
                        ...
                        # Fix AttributeError: Ensure key is unique and state key is correct
...
# ==================================
    ...
    # Mengambil data dari state yang benar
