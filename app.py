...
<style>
    ...
    /* TILES & BALLOONS */
    .metric-tile { padding: 20px; border-radius: 12px; color: white; margin-bottom: 15px; display: flex; align-items: center; gap: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
    ...
    .source-box-ui { background-color: #e3f2fd !important; padding: 15px !important; border-radius: 10px !important; border-left: 6px solid #0d47a1 !important; margin-bottom: 25px !important; }
    ...
    div.stButton > button[key^="btn_"] { background: transparent !important; color: #0d47a1 !important; font-size: 16px !important; font-weight: 800 !important; padding: 0px !important; text-align: left !important; box-shadow: none !important; margin-bottom: -5px !important; }
</style>
...
if logo_b64: st.sidebar.markdown(f'<div style="display: flex; align-items: center; gap: 12px; padding-bottom: 20px;"><img src="data:image/png;base64,{logo_b64}" width="45"><span style="font-size: 18px; font-weight: 800; color: white;">SI-PANDAI</span></div>', unsafe_allow_html=True)
...
if st.session_state.page_view == "dashboard":
    ...
    if kab_pilih != "Semua": df_f = df_f[df_f["kab_kota"] == kab_pilih]
    ...
    val_APS = f"{(df_f['jumlah_siswa'].sum() / df_f['jumlah_penduduk'].sum() * 100):.2f}%" if df_f['jumlah_penduduk'].sum() > 0 else "0%"
    ...
    with cv1: st.subheader("🗺️ Peta Sebaran ATS"); st.map(df_f, latitude="lat", longitude="lon", use_container_width=True)
    with cv2: st.subheader("📊 Grafik ATS Wilayah"); st.plotly_chart(px.bar(df_f.sort_values("ats_disabilitas", ascending=False).head(10), x='ats_disabilitas', y='kab_kota', orientation='h', color_continuous_scale='Blues'), use_container_width=True)
