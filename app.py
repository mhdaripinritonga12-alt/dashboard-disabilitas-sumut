...
def load_all_data():
    try:
        # Load data wilayah (ATS)
        df_ats = pd.read_csv("master_data_si_pandai.csv")
        # Load data sekolah (Sarpras)
        df_sch = pd.read_csv("master_data_sekolah1.csv")
        
        # Bersihkan nama kolom agar tidak KeyError (kab_kota)
...
# Pastikan kolom kab_kota ada
target_col = "kab_kota"
if not data_wilayah.empty:
    opsi = ["Semua"] + sorted(data_wilayah[target_col].unique().tolist())
...
if st.session_state.page_view == "dashboard":
    ...
    if kab_pilih != "Semua":
        df_f = df_f[df_f[target_col] == kab_pilih]

    # Matriks Capaian
    m1, m2, m3, m4 = st.columns(4)
    with m1: st.metric("Penduduk Disabilitas", f"{int(df_f.iloc[:,1].sum()):,}")
    with m2: st.metric("Siswa Belajar", f"{int(df_f.iloc[:,2].sum()):,}")
    with m3: st.metric("Anak Tidak Sekolah", f"{int(df_f.iloc[:,3].sum()):,}")
    with m4:
    ...
    with cv1:
        ...
        if kab_pilih == "Semua":
            if not data_sekolah.empty:
                st.map(data_sekolah, latitude="latitude", longitude="longitude", use_container_width=True)
        else:
            # Tampilkan semua sekolah dalam satu kabupaten sekaligus
            df_map = data_sekolah[data_sekolah[target_col] == kab_pilih].copy()
            if not df_map.empty:
                st.map(df_map, latitude="latitude", longitude="longitude", use_container_width=True)
            else:
                st.info("Data koordinat sekolah di wilayah ini belum tersedia.")
