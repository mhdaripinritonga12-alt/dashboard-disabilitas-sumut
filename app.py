...
if "selected_kab" not in st.session_state: st.session_state.selected_kab = None
...
# ==================================
# Bagian 1: CSS CUSTOM (LOCKED STYLE)
# ==================================
st.markdown("""
<style>
...
# MENGHAPUS OPSI "SEMUA"
opsi = sorted(data_wilayah[col_kab].unique().tolist()) if not data_wilayah.empty else []
...
if st.session_state.page_view == "dashboard":
    ...
    df_f = data_wilayah[data_wilayah[col_kab] == kab_pilih] if not data_wilayah.empty else pd.DataFrame()

    # Matriks
    m1, m2, m3, m4 = st.columns(4)
    if not df_f.empty:
        val_p = f"{int(df_f.iloc[:,1].sum()):,}"
        val_s = f"{int(df_f.iloc[:,2].sum()):,}"
        val_a = f"{int(df_f.iloc[:,3].sum()):,}"
        val_APS = f"{(int(df_f.iloc[:,2].sum()) / int(df_f.iloc[:,1].sum()) * 100):.2f}%" if int(df_f.iloc[:,1].sum()) > 0 else "0%"
    else:
        val_p = val_s = val_a = "0"
        val_APS = "0%"
    ...
    with cv1:
        st.subheader("🗺️ Lokasi Wilayah")
        if not df_f.empty: st.map(df_f, latitude="lat", longitude="lon", use_container_width=True)
    with cv2:
        st.subheader("📊 Statistik Wilayah")
        if not df_f.empty: st.plotly_chart(px.bar(df_f, x=df_f.columns[3], y=col_kab, orientation='h', color_discrete_sequence=['#1565c0']), use_container_width=True)

    st.divider()
    with st.expander("📋 Lihat & Download Data Tabel"):
        st.dataframe(df_f, use_container_width=True)
        csv = df_f.to_csv(index=False).encode('utf-8')
        st.download_button(label="Download Data sebagai CSV 📥", data=csv, file_name=f'data_{kab_pilih}.csv', mime='text/csv')
...
# --- LAINNYA ---
    ...
    st.divider()
    st.info("Halaman ini berisi kebijakan Pendidikan Khusus di Sumatera Utara.")
